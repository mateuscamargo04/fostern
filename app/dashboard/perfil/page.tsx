...
export default function PerfilPage() {
  ...
  const salvar = async (event: FormEvent) => {
    ...
    try {
      ...
      const { error: dbErr } = await supabase.from("perfis").update({ nome: nome.trim(), email }).eq("id", user!.id);
      if (dbErr) throw dbErr;
      window.dispatchEvent(new Event(USER_UPDATED_EVENT));
      notificar("Perfil atualizado.", "ok");
    } catch (err) {
    ...
    <div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <motion.section {...fade} transition={{ duration: 0.7, delay: 0.1, ease: [0.22, 1, 0.36, 1] }} className="rounded-lg border border-mist bg-white p-5 md:p-6">
          ...
          <div className="mt-6 flex items-center gap-5">
            {user?.avatarUrl ? (
              ...
              <span className="grid h-20 w-20 shrink-0 place-items-center rounded-full bg-navy font-serif text-[28px] font-semibold text-gold">
                {user?.initials ?? "ES"}
              </span>
            )}
        ...
        <motion.form
          ...
          className="rounded-lg border border-mist bg-white p-5 md:p-6 lg:col-span-2"
        >
          ...
          <p className="mt-1.5 text-[12px] leading-5 text-graphite/55">
            O nome é usado na saudação e na assinatura das suas mentoriais.
          </p>
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-2">
            ...
            <div>
              ...
              <input id="perfil-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" className={inputClass} />
              <p className="mt-2 text-[10px] leading-4 text-graphite/45">Trocar o e-mail envia uma confirmação para o novo endereço.</p>
            </div>
          </div>
